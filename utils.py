...
from torch_geometric.utils import dense_to_sparse, dropout_adj\
    # ,true_positive, true_negative, false_positive, false_negative, precision, recall, f1_score
...
def generate_dataset(pca_feature_dimension, feature_path, feature_name_path, slide_patch_adj_path, use_knn_graph, pca_right_now):
    ...
    with open(path, "rb") as f:
        dic_slide_feature = json.loads(f.read())

    # load slide:[patch]
    path = feature_name_path
    with open(path, "rb") as f:
        dic_slide_patch = json.loads(f.read())

    # 邻接矩阵
    path = slide_patch_adj_path
    with open(path, "rb") as f:
        dic_slide_adj_info = json.loads(f.read())
    ...
    dic_slide_patch_use = get_first_n_items(dic_slide_patch, 20)

    # keys = ['TCGA-44-2664-01A-01-TS1','TCGA-05-4389-01A-01-BS1']
    #
    # dic_slide_feature = {key: dic_slide_feature[key] for key in keys if key in dic_slide_feature}
    # dic_slide_patch = {key: dic_slide_patch[key] for key in keys if key in dic_slide_patch}
    # dic_slide_adj_info = {key: dic_slide_adj_info[key] for key in keys if key in dic_slide_adj_info}
    ...
    for key, patch in dic_slide_patch.items():
        ...
        # 将字符串转换为 ASCII 编码
        ascii_values = [ord(c) for c in ASCII]

        # 将 ASCII 编码转换为 PyTorch 张量
        data.data_name = torch.tensor(ascii_values, dtype=torch.int)
...
def get_first_n_items(original_dict, n):
    # 创建一个字典的键的迭代器
    key_iterator = iter(original_dict)

    # 创建一个新的空字典
    new_dict = {}

    # 获取前n个键，并将它们和对应的值添加到新的字典中
    for _ in range(n):
        try:
            key = next(key_iterator)
            new_dict[key] = original_dict[key]
        except StopIteration:
            # 如果原始字典中的键的数量少于n，就会引发StopIteration异常
            break
