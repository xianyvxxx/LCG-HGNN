...
def main():
    ...
    # 预处理临床图像（只做一次）
    clinical_cache = os.path.join(os.path.dirname(output_file), "clinical_features.npz")
    ...
    similarities_genes_dict = defaultdict(list)
    similarities_genes_mean_dict = defaultdict(list)
    genes = ['TNR', 'RELN', 'LAMA2', 'COL6A3', 'TP53', 'KRAS', 'STK11', 'EGFR', 'COL11A1', 'COL22A1']
    similarities_genes = [[] for _ in genes]  # 初始化二维列表
    for filename_PRE in tqdm(os.listdir(predicted_img_PATH), desc="处理预测图像"):
        ...
        for filename, similarities_gene in zip(clinical_filenames, similarities):
            for i, gene in enumerate(genes):
                if gene in filename:
                    similarities_genes[i].append(similarities_gene)
        # 存储排序后的列表和平均值
        sorted_similarities = defaultdict(list)
        average_similarities = defaultdict(list)

        for values, gene in zip(similarities_genes, genes):
            sorted_similarities[gene] = sorted(float(x) for x in values)  # 升序排序
            average_similarities[gene] = np.mean(values)

        similarities_genes_dict[filename_PRE] = sorted_similarities
        similarities_genes_mean_dict[filename_PRE] = average_similarities
        # 记录结果
        results.append((filename_PRE, similarities.tolist()))

        # 打印统计信息
        mean_sim = np.mean(similarities)
        mean_sim_list.append((filename_PRE,mean_sim))
    ...
    gene_centric_dict = defaultdict(list)

    # 遍历原始字典
    for filename_PRE, gene_data in similarities_genes_mean_dict.items():
        for gene, mean_val in gene_data.items():
            if mean_val is not None:
                gene_centric_dict[gene].append((filename_PRE, mean_val))

    # 转回普通 dict（可选）
    gene_centric_dict = dict(gene_centric_dict)

    for gene in gene_centric_dict:
        gene_centric_dict[gene] = sorted(gene_centric_dict[gene], key=lambda x: x[1], reverse=True)
    # 一次性写入所有结果
    ...
    mean_sim_list.sort(key=lambda x: x[1], reverse=True)
    print(f"\n✅ 平均相似度排序情况:{mean_sim_list}")
    # 一次性写入所有结果
    with open(r"G:\data\normlize\no_ke_mean_sort_prototype.txt", 'w', encoding='utf-8') as f:
        for filename_sort, mean_sim_sort in mean_sim_list:
            # 格式: 预测图像文件名: [sim1, sim2, ...]
            f.write(f"{filename_sort}: {mean_sim_sort}\n")

    with open(r"G:\data\normlize\no_ke_gene_prototype.txt", 'w', encoding='utf-8') as f:
        for filename, items in similarities_genes_dict.items():
            f.write(f"Group: {filename}\n")
            for gene_name, item in items.items():
                f.write(f"{gene_name}: {item}\n")
    with open(r"G:\data\normlize\no_ke_mean_gene_prototype.txt", 'w', encoding='utf-8') as f:
        for filename, items in gene_centric_dict.items():
            f.write(f"Group: {filename}\n")
            for gene_name, item in items:
                f.write(f"{gene_name}: {item}\n")
