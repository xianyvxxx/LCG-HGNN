...
pathway = ['R-HSA-1474244']#hsa04012\hsa04014\ hsa05161\hsa04310\hsa04151\hsa04820\hsa04512\R-HSA-8874081, R-HSA-8875878, R-HSA-6806834\R-HSA-1474290, R-HSA-8948216\R-HSA-1442490\R-HSA-1474228\hsa04020\R-HSA-1474244\R-HSA-1474290\R-HSA-8948216\hsa04814\R-HSA-3000171\R-HSA-3906995
##############################################################################################
...
def train(train_loader, epoch, model, optimizer, device, init_lr, num_epochs, index_hub, lambda_reg=0.03):
    ...
    for data_list in train_loader:
        ...
        # 计算误差，假设 result_output 和 y 的形状是 (20, 3)
        error = torch.abs(result_output - y)  # 误差形状为 (20, 3)

        # 确保 error 和 edge_attr 形状匹配（每个batch的每个预测值与对应的边权重相乘）
        # self_graph.edge_attr 形状为 (3, 1)，我们可以通过转置的方式与 error 的形状对齐
        adjustment = error * self_graph.edge_attr.T  # (20, 3) * (3, 1) -> (20, 3)

        # 计算每个边的更新值，可以按需求使用平均、求和等方式进行综合
        adjusted_error = adjustment.sum(dim=0)  # 对所有batch进行求和，得到一个 (3,) 的向量

        # 计算adjusted_error的最大绝对值
        max_abs_error = adjusted_error.abs().max()

        # 确保最大值在1的范围内进行等比例缩放
        if max_abs_error > 0:  # 避免除以0
            adjusted_error = adjusted_error / max_abs_error  # 将所有值缩放到[-1, 1]范围内

        # 计算adjusted_error的长度
        length = adjusted_error.numel()  # length 即 adjusted_error 的元素个数

        adjust_factors = torch.full((num_classes, 1), 1 / length, dtype=torch.float32)

        new_value = 2 / length  # 你想要的新值
        ...
        updated_edge_attr = self_graph.edge_attr.clone()  # 克隆 self_graph.edge_attr
        ...
        # print('新增边权重变化：',self_graph.edge_attr)
    ...
    for name, param in model.named_parameters():
        if param.grad is not None:
            grad_norm = param.grad.norm(2)  # L2范数
            print(f"{name} - Gradient Norm: {grad_norm}")
            save_Gradient.append(f"{name} - Gradient Norm: {grad_norm}")
...
for gene, task in zip(pathway, pathway_task_list):
    ...
    def init_weights(m):
        # 对于 Linear 层，使用 Xavier 初始化
        if isinstance(m, nn.Linear):
            init.xavier_uniform_(m.weight)  # 使用 Xavier 初始化权重
            if m.bias is not None:
                init.zeros_(m.bias)  # 偏置初始化为 0

        # 对于 GCNConv 层，使用 Xavier 初始化
        elif isinstance(m, GCNConv):
            # 初始化 GCNConv 层的内部线性层权重
            init.xavier_uniform_(m.lin.weight)  # 使用 Xavier 初始化权重
            if m.lin.bias is not None:
                init.zeros_(m.lin.bias)  # 偏置初始化为 0

        # 对于 GINConv 层，使用 Xavier 初始化
        elif isinstance(m, GINConv):
            # 如果 GINConv 中的 mlp 层存在
            if hasattr(m, 'mlp'):
                for layer in m.mlp:
                    if isinstance(layer, nn.Linear):
                        init.xavier_uniform_(layer.weight)  # 使用 Xavier 初始化权重
                        if layer.bias is not None:
                            init.zeros_(layer.bias)  # 偏置初始化为 0

        # 对于卷积层，使用 Kaiming 初始化
        elif isinstance(m, nn.Conv2d) or isinstance(m, nn.Conv1d):
            init.kaiming_uniform_(m.weight, nonlinearity='relu')  # 使用 Kaiming 初始化
            if m.bias is not None:
                init.zeros_(m.bias)

        # 对于批归一化层，初始化权重为 1，偏置为 0
        elif isinstance(m, nn.BatchNorm2d) or isinstance(m, nn.BatchNorm1d):
            init.ones_(m.weight)  # 批归一化的权重初始化为 1
            init.zeros_(m.bias)  # 批归一化的偏置初始化为 0

        # 对于边属性 (edge_attr)，初始化为随机值
        elif isinstance(m, nn.Parameter):  # 对边属性初始化
            init.normal_(m, mean=0, std=0.1)  # 使用正态分布初始化边属性
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(useful_samples, labels.argmax(axis=1))):  # 对标签取最大值进行分层
        print(f"\nFold {fold + 1}:")
        ...
        model = Net(num_classes,24, edge_values)  # .to(device)
        ...
        scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=init_lr * 3, steps_per_epoch=len(train_loader),
                                                        epochs=num_epochs)
        ...
        for epoch in range(num_epochs):
            ...
            train_epoch_data_save_dir = '/mnt/data16t/yuy/pathway/patch/only_change/GIN_change/train_epoch_data_RHSA1474244'
            os.makedirs(train_epoch_data_save_dir, exist_ok=True)  # 创建目录（如果不存在）
            test_epoch_data_save_dir = '/mnt/data16t/yuy/pathway/patch/only_change/GIN_change/test_epoch_data_RHSA1474244'
            os.makedirs(train_epoch_data_save_dir, exist_ok=True)  # 创建目录（如果不存在）

            torch.save(train_epoch_data, f'/mnt/data16t/yuy/pathway/patch/only_change/GIN_change/train_epoch_data_RHSA1474244/{fold + 1}_{epoch+ 1}.pth')
            torch.save(test_epoch_data, f'/mnt/data16t/yuy/pathway/patch/only_change/GIN_change/test_epoch_data_RHSA1474244/{fold + 1}_{epoch+ 1}.pth')
            ...
            save_path = '/mnt/data16t/yuy/pathway/patch/only_change/GIN_change/model_RHSA1474244/task_{}/gene_{}/model/flod_{}/'.format(
                task, gene, k)
    ...
    with open('/mnt/data16t/yuy/pathway/patch/only_change/GIN_change/train_output_RHSA1474244.txt', 'w') as f:
